...
- **Геометрическая интерпретация**: Представь пространство признаков как лист бумаги. Дерево делает "разрезы" (разделения), деля его на прямоугольники (в 2D) или гиперкубы (в многомерном случае). Каждый разрез определяется условием, например, "x₁ ≤ 5".
...
- **Линейные модели**: Быстрее, проще, но ограничены линейными зависимостями.
